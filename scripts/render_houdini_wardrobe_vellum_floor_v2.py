#!/usr/bin/env python3
"""Render a textured, floor-draped Houdini Vellum v2 proof in Blender."""
...
if not all(len(obj.data.uv_layers) > 0 for obj in garment_objects):
    raise RuntimeError("Houdini v2 OBJ lost its UV coordinates")
...
medium_scale = horizontal_extent * 1.46
render_still(
    camera,
    center + Vector((0.0, 0.0, extent.z * 0.10)),
    medium_scale,
    medium_path,
)
camera.location = center + Vector(
...
render_still(
    camera,
    center + Vector((0.0, 0.0, extent.z * 0.06)),
    medium_scale * 0.66,
    closeup_path,
)
